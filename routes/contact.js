...
const handleContact = async (req, res) => {
  try {
    ...
    await query(
      `INSERT INTO contact_messages (id, name, email, phone, subject, message, user_id, status) 
...
router.get("/", authenticateFirebaseUser, async (req, res) => {
  try {
    ...
    let baseQuery = `FROM contact_messages WHERE 1=1`;
    const params = [];
    ...
    if (search) {
      params.push(`%${search}%`);
      baseQuery += ` AND (name ILIKE $${params.length} OR email ILIKE $${params.length} OR subject ILIKE $${params.length} OR message ILIKE $${params.length})`;
    }

    const countRes = await query(`SELECT COUNT(*) ${baseQuery}`, params);
    const total = parseInt(countRes.rows[0].count, 10);

    const allowedSortFields = ["created_at", "updated_at", "status", "name", "email"];
...
router.get("/:id/notes", authenticateFirebaseUser, async (req, res) => {
  try {
    const { id } = req.params;
    const { rows } = await query(
      `SELECT * FROM support_notes WHERE entity_type = 'contact' AND entity_id = $1 ORDER BY created_at ASC`,
      [id]
    );
    res.json(rows);
  } catch (error) {
